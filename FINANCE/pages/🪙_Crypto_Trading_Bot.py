...
st.set_page_config(page_title="Crypto Trading Bot",layout="wide",page_icon="🪙")
...
def get_ai_recommendations(predicted_prices):
    try:
        ...
        prompt = f"""
        The following are predicted cryptocurrency prices for the next {len(predicted_prices)} days:
        {predicted_prices}
...
# Streamlit App
def main():
    st.title("Cryptocurrency Trading Bot using LSTM")
    
    # Sidebar for user input
    st.sidebar.header("User Input")
    ticker = st.sidebar.text_input("Cryptocurrency Ticker (e.g., BTC-USD, ETH-USD)", value="BTC-USD")
    ...
    if st.sidebar.button("Run Prediction"):
        if ticker:
            try:
                ...
                st.write(f"Latest Cryptocurrency Data for {ticker}")
                st.write(data.tail())
            ...
            st.error("Please enter a valid cryptocurrency ticker!")
